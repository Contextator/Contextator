/**
 * `src/db/ensure-schema.ts` as it stood at `7ab2017`, the last commit before generated migrations
 * ([ADR-0033](../../../.ssot/ADR.md#adr-0033)). Copied verbatim; only the two import paths are
 * rewritten to reach out of `test/integration/fixtures/` instead of out of `src/db/`.
 *
 * **This file is never to be edited.** It is not a second implementation to keep working — it is the
 * frozen definition of "what the schema used to be", and the whole review of the change that replaced
 * it is `schema-equivalence.itest.ts`: this function on one database, `bootstrapDatabase` on another,
 * and the two catalogue projections compared as text. Editing it to make that comparison pass would
 * delete the only evidence there is. When the last installation that could be running this DDL is
 * gone, delete the file and the test together rather than letting either drift.
 */
