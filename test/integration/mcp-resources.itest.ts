...
/**
 * Twice, once per value of `MCP_STRUCTURED_OUTPUT`: the resources do not depend on it
 * ([ADR-0087](../../.ssot/ADR.md#adr-0087)), and the factory is where a flag could have leaked into them.
 */
describe.each([
  ['off', '0'],
  ['on', '1'],
])('over the real endpoint, structured output %s', (state, flag) => {
  ...
  beforeAll(async () => {
    live = await startMcpInstance(database, {
      ...
      docRoot: root,
      env: { MCP_STRUCTURED_OUTPUT: flag },
    });
  });
  ...
  it("lists, pages and reads this project's documents, and refuses another's, through the router", async () => {
    ...
    try {
      ...
      // The flag did reach this instance: the tools publish an output schema exactly when it is on.
      const { tools } = await client.listTools();
      for (const tool of tools) expect(tool.outputSchema !== undefined).toBe(flag === '1');
