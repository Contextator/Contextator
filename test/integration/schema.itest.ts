...
describe('a 0.1 database, along the route ADR-0033 documents', () => {
  ...
  it('reaches schema 5 under the 0.1.0 ladder, and the bootstrap then adopts it and migrates it', async () => {
    ...
    // for the reason `^search_quer` and `^oauth_clients` are, and `0012_lexical_configurations`
    // ([ADR-0064](../../../.ssot/ADR.md#adr-0064)), which adds `chunks.text_search_config` and
    // rebuilds `chunks_project_generation_idx` around it. So the claim is no
