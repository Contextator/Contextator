...
| `TRUST_PROXY` | `0` | **Which peers may tell this server where a request came from.** `req.ip` is what the per-IP sign-in limit, `/oauth/register`'s per-host budget and the address beside an audit event are keyed on. `0` reads the socket's own peer address and ignores `X-Forwarded-For`, which is right for the shipped `docker compose` shape — nothing is in front of it, so a caller writing that header would otherwise be choosing its own rate-limit key. **Put a reverse proxy in front and you must set this**, or every request looks like the proxy and the per-IP sign-in limit becomes an instance-wide one. Name the proxy rather than trusting all of them: an IP, a CIDR block, or the subnet names `loopback` / `linklocal` / `uniquelocal`, comma-separated (`TRUST_PROXY=loopback`, `TRUST_PROXY=10.0.0.0/8,172.18.0.0/16`). `1` trusts whoever wrote the header and is only safe when nothing but the proxy can open a socket to this port. A hop count is **not** accepted — it is a claim the server cannot check and goes silently wrong the day a CDN appears in front of the proxy |
