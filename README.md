...
It deploys exactly one Pod — this chart does not expose a `replicaCount` value, and its values schema
rejects any value other than 1, because MCP sessions and the indexer both live in that one process
...
set it to a `-slim` tag (`<version>-slim`, `<major>.<minor>-slim` or `latest-slim`, published from the
release after `v0.1.0` on; for `v0.1.0` itself, build the `slim` target yourself). See
...
charts/contextator/           Helm chart for Kubernetes: one Pod, the `-slim` image, an external PostgreSQL; values.schema.json refuses a second replica
